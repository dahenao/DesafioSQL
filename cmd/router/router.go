...
func (r *router) buildCustomersRoutes() {
	...
	c := r.rg.Group("/customers")
	{
		...
		c.POST("", handler.Post())
		c.POST("/batch", handler.BatchPost())
	}
}
...
func (r *router) buildInvoicesRoutes() {
	...
	i := r.rg.Group("/invoices")
	{
		...
		i.POST("", handler.Post())
		i.POST("/batch", handler.BatchPost())
		i.GET("/UpdateTotal", handler.BatchPost())
	}
}
...
func (r *router) buildProductsRoutes() {
	...
	p := r.rg.Group("/products")
	{
		...
		p.POST("", handler.Post())
		p.POST("/batch", handler.BatchPost())
	}
}
...
func (r *router) buildSalesRoutes() {
	...
	s := r.rg.Group("/sales")
	{
		...
		s.POST("", handler.Post())
		s.POST("/batch", handler.BatchPost())
	}
}
